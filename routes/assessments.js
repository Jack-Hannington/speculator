...
router.get('/getResponseDetails/:responseSetId', ensureAuthenticated, async (req, res) => {
  const { responseSetId } = req.params;
  const userId = req.user.id;

  try {
    // Step 1: Verify that the responseSetId belongs to the authenticated user directly in the query
    const { data: responseSet, error: ownershipError } = await supabase
      .from('user_responses')
    ...
    if (ownershipError || !responseSet) {
      console.error('Ownership verification failed or access denied.');
